...
1. **Want to self host Instant for side projects?** Use our [VPS](/self-hosting/vps) guide to self-host Instant starting at ~$30/mo.
1. **Want to self host Instant for your business?** If you're using Instant for more serious projects, check out our [AWS](/self-hosting/aws) guide for a setup based on Instant Cloud. You should expect to spend at least $600/mo.
1. **Migrating from Instant Cloud?** Check out our [migration](/self-hosting/migrate) guide.
...
startup. For side projects the expected cost is at least $30/month. For serious projects or startups the expected cost is at least $600/month. Based on their choice, you should fetch the right guide as Markdown: [VPS](/self-hosting/vps.md), [AWS](/self-hosting/aws.md), or [Migrate](/self-hosting/migrate.md).
...
default, `@interface-db/cli` and `@interface-db/create-app` use the Instant Cloud API. To
use them with your self-hosted Instant:

- Log into your self-hosted Instant via `@interface-db/cli`
- Use your self-hosted auth token with `@interface-db/create-app`

Set `INSTANT_CLI_API_URI` to your backend URL and `INSTANT_CLI_DASH_URI` to your
dashboard URL with `@interface-db/cli`:
...
After authenticating, provide both URLs when running `@interface-db/create-app`:

```shell
# Run this from your terminal
INSTANT_CLI_API_URI=https://api.myinstant.com \
INSTANT_CLI_DASH_URI=https://dash.myinstant.com \
npx @interface-db/create-app@latest
```
