...
[`@interface-db/resumable-stream`](https://www.npmjs.com/package/@interface-db/resumable-stream) is a drop-in replacement for Vercel's `resumable-stream` library that supports resuming ongoing streams after page reloads. It requires no Redis instance, and your streams never expire.
...
import { id as generateId } from '@interface-db/react';
...
import { after } from 'next/server';
import { createResumableStreamContext } from '@interface-db/resumable-stream';
import { id as generateId } from '@interface-db/admin';
...
If you want to jump into building an app, `@interface-db/create-app` has a working template that uses this pattern:

```sh
npx @interface-db/create-app@latest --base ai-chat
```
