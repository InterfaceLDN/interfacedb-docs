...
1. Authenticate with Instant in your terminal via `npx @interface-db/cli login`.
1. Create new projects via `npx @interface-db/create-app`.
1. Push changes to your schema and permissions via `npx @interface-db/cli push`.
1. Use the [Sandbox](https://www.instantdb.com/dash?t=sandbox) to debug queries,
   transactions, and permissions.
1. When you're ready for production, [restrict creating](/patterns#restrict-creating-new-attributes) new attributes.
1. If you need more help, check out our [patterns page](/patterns) for common
   recipes.
...
Use the [Instant CLI](/cli) to authenticate with Instant in your
...
[create-instant-app](/create-instant-app). This will give you some starter code and set up rules for your LLM
agent. If your agent supports it, we also recommend setting up the [Instant MCP Server](/using-llms#instant-mcp-server).
...
[adding auth](/auth/magic-codes.md).
...
edits to your local schema and permission files, and then run `npx @interface-db/cli push` to push changes to your project.
...
into your local files, run `npx @interface-db/cli pull`.
...
Huzzah, you're ready for prime time! When you go to production, be sure to [restrict creating](/patterns#restrict-creating-new-attributes) new
attributes. You can also
consider setting up [separate apps](/patterns#managing-local-vs-production-apps) for local development and
production.
...
We've also made a [patterns page](/patterns) with common recipes for using
InstantDB.
