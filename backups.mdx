...
title: Backups
description: >-
  How Instant creates backups, how to download them, and what a backup archive
  contains.
---
...
```sh
npx @interface-db/cli@latest backup list
```
...
```sh
npx @interface-db/cli@latest backup download --latest
```
...
Read the [Self hosting](/self-hosting) guide for more information on how to set up a self-hosted instance.

Ensure that you've set the [deployment superuser](/self-hosting#configure-the-deployment-superuser) via the `INSTANT_SUPERUSER_EMAIL` environment variable.
...
npx @interface-db/cli@latest backup list --json

# Download a backup (interactive picker when no backup is given)
npx @interface-db/cli@latest backup download

# Download the most recent backup
npx @interface-db/cli@latest backup download --latest

# Download a specific backup to a path of your choosing
npx @interface-db/cli@latest backup download <backup-id> --out my-backup.zip
```

All commands read the app ID from your `.env` file (`INSTANT_APP_ID`, or a framework-specific variant like `NEXT_PUBLIC_INSTANT_APP_ID`; see [App ID](/cli#app-id)). Pass `--app <app-id>` to target a different one, and run `--help` on any command for the full list of flags.
