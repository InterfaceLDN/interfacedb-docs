...
```bash
npx @interface-db/cli init-without-files --title my-new-app --temp
```
...
```typescript
import { PlatformApi, i } from '@interface-db/platform';
...
```bash
npx @interface-db/cli init-without-files --title my-long-lived-app --token YOUR_TOKEN
```
...
If you are using the CLI, you'll have your `instant.schema.ts` and `instant.perms.ts` files in your directory. Run the [standard](/cli) CLI push command with your token:
...
```bash
npx @interface-db/cli push schema --token YOUR_TOKEN
```
...
```bash
npx @interface-db/cli push schema --token YOUR_ADMIN_TOKEN
```
