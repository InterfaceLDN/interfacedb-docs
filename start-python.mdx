...
```shell
npx @interface-db/create-app --python my-app
cd my-app
uv sync
```
...
See [Modeling data](/modeling-data) for the full schema reference.
...
See [Writing data](/instaml) for the full mutation reference.
...
See [Streams](/streams) for the full reference.
...
See [Storage](/storage) for the broader storage model.
...
[Presence, Cursors, and Activity](/presence-and-topics).
...
See [Webhooks](/webhooks) for the broader webhook model.
