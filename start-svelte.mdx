...
The fastest way to get started with Instant and SvelteKit is to use @interface-db/create-app to scaffold a new project with Instant already set up.

To get started, run:

```shell
npx @interface-db/create-app --sv
```
...
```ts
import { init } from '@interface-db/svelte';
...
```ts
import { i } from '@interface-db/svelte';
...
```svelte
<script lang="ts">
  import { id, type InstaQLEntity } from '@interface-db/svelte';
...
```svelte
<script lang="ts">
  import { id } from '@interface-db/svelte';
  import { db } from '$lib/db';
...
To learn more, see our [writing data](/instaml) docs.

## Auth

The Svelte SDK supports all of Instant's [auth methods](/auth): [magic codes](/auth/magic-codes), [guest auth](/auth/guest-auth), [Google OAuth](/auth/google-oauth), and more.
...
```svelte
<script lang="ts">
  import { SignedIn, SignedOut } from '@interface-db/svelte';
  import { db } from '$lib/db';
</script>
...
```svelte
<script lang="ts">
  import { Cursors } from '@interface-db/svelte';
  import { db } from '$lib/db';
...
The `Cursors` component supports custom cursor rendering via a `renderCursor` snippet, a configurable wrapper element (`as`), and `className`/`style` props for styling. See the [Presence, Cursors, and Activity](/presence-and-topics) docs for more details.
