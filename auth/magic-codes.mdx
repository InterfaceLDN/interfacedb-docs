...
```typescript
import { init, type User } from '@interface-db/core';
...
The fields must be defined as optional attributes on `$users` in your schema. A `create` rule on `$users` is also required. See [Setting properties at signup](/users#setting-properties-at-signup) for the full guide.
