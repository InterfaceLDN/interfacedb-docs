...
- Click _Configure_
- Select _Primary App ID_ from Step 1
- To _Domains_, add `api.interfacedb.com`
- To _Return URLs_, add `https://api.interfacedb.com/runtime/oauth/callback`
- Click _Continue_ → _Save_
...
<Accordion title="Other authentication methods">
...
```shell
npx @interface-db/cli@latest auth origin add --type website --url myapp.com
```
...
import { init, tx } from '@interface-db/react-native';
