...
The fastest way to get started with Instant on TanStack Start is to use npx @interface-db/create-app to scaffold a new project with Instant already set up.

To get started, run:

```shell
npx @interface-db/create-app -b tanstack-start
```
...
Use `@interface-db/cli` to set up a new Instant project. This will prompt you to log in if you haven't already. It will then create schema and permissions files and update your `.env` file.
...
import { id, InstaQLEntity } from "@interface-db/react";
...
Want to dive deeper on how this todo app works? Check out our step-by-step [Todo List Tutorial](https://interfacedb.com/examples/todos). In this tutorial we walk through how to build a todo list app from scratch, and explain how the queries and transactions work in more detail.

For the advanced use case of integrating with TanStack Query and enabling SSR, refer to our [tanstack-start-with-tanstack-query](https://github.com/instantdb/instant/tree/main/examples/tanstack-start-with-tanstack-query) example. It can be scaffolded using `npx @interface-db/create-app -b tanstack-start-with-tanstack-query`.
