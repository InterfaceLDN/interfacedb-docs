...
With that, you can use `db` to [write data](/instaml), [make queries](/instaql), [handle auth](/auth), and more!
...
```typescript
import { init, i } from '@interface-db/react';
...
- **websocketURI?**: Custom WebSocket endpoint for real-time connections. Defaults to `'wss://api.interfacedb.com/runtime/session'`. Change this for connecting to development or self-hosted instances.

- **apiURI?**: Custom HTTP API endpoint for auth and storage operations. Defaults to `'https://api.interfacedb.com'`. Change this for connecting to development or self-hosted instances.
