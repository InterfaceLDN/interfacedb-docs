...
start a new project with `@interface-db/create-app` you'll get these rules
automatically. If you have an existing project you can add the rules manually.

[Save these rules](https://interfacedb.com/llm-rules/AGENTS.md) at the root of your project as:
...
particular functionality. For example, here are the recommended docs for [adding auth](/auth/magic-codes.md).
...
We built [`@interface-db/mcp`](https://github.com/instantdb/instant/tree/main/client/packages/mcp) to enable creating, managing, and updating your Instant apps.
...
We recommend using our hosted MCP server, but we also support running [`@interface-db/mcp`](https://github.com/instantdb/instant/tree/main/client/packages/mcp) locally
...
```json
{
  "mcpServers": {
    "instant": {
      "command": "npx",
      "args": ["-y", "@interface-db/mcp", "--token", "<token>"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "instant": {
      "command": "cmd",
      "args": ["/c", "npx", "-y", "@interface-db/mcp", "--token", "<token>"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "instant": {
      "command": "wsl",
      "args": ["npx", "-y", "@interface-db/mcp", "--token", "<token>"]
    }
  }
}
```
