...
If your backend is written in JavaScript, you can use the [`@interface-db/admin`](/backend) SDK to connect your server to Instant.
...
If you give this documentation to your AI agent, it can create a custom SDK for your backend language. Here's the [Markdown](/http-api.md).
...
```shell
curl -X POST "https://api.interfacedb.com/admin/query" \
...
If you need [rule params](/permissions#rule-params), include `$$ruleParams` at the top-level:
...
```shell
curl -X POST "https://api.interfacedb.com/admin/transact" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
...
map to the [Instant transactions](/instaml) you know:
...
```shell
curl -N -X POST "https://api.interfacedb.com/admin/subscribe-query" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
  -d '{"query":{"tasks":{}}}'
...
```shell
# By email!
curl -X GET "https://api.interfacedb.com/admin/users?email=alyssa_p_hacker@instantdb.com" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID"

# By id
curl -X GET "https://api.interfacedb.com/admin/users?id=$USER_ID" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID"

# Or by a refresh token
curl -X GET "https://api.interfacedb.com/admin/users?refresh_token=$REFRESH_TOKEN" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID"
```
...
```shell
# By email
curl -X DELETE "https://api.interfacedb.com/admin/users?email=alyssa_p_hacker@instantdb.com" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID"

# By id
curl -X DELETE "https://api.interfacedb.com/admin/users?id=$USER_ID" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID"

# Or by an auth token
curl -X DELETE "https://api.interfacedb.com/admin/users?refresh_token=$REFRESH_TOKEN" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID"
```
...
curl -X POST "https://api.interfacedb.com/admin/sign_out" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
...
```shell
# By email
curl -X POST "https://api.interfacedb.com/admin/refresh_tokens" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
  -d '{"email":"alyssa_p_hacker@instantdb.com"}'
...
user for you. You can pass `extra-fields` to set custom `$users` properties on creation. The response includes `user.refresh_token` and `"created": true` when a new user is created. You can pass this token on to your client and use it to [log in](/backend#2-frontend-db-auth-sign-in-with-token).

## Custom magic codes

We support a [magic code flow](/auth) out of the box. However, if you'd like to use your own email provider to send the code, you can create a magic code with `POST /admin/magic_code`:

```shell
curl -X POST "https://api.interfacedb.com/admin/magic_code" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
  -d '{"email":"alyssa_p_hacker@instantdb.com"}'
...
```shell
curl -X POST "https://api.interfacedb.com/admin/send_magic_code" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
  -d '{"email":"alyssa_p_hacker@instantdb.com"}'
...
```shell
curl -X POST "https://api.interfacedb.com/admin/verify_magic_code" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
...
```shell
curl -X POST "https://api.interfacedb.com/runtime/auth/verify_refresh_token" \
  -H "Content-Type: application/json" \
...
You can also manage your app's [storage](/storage) with the HTTP API.
...
```shell
curl -X PUT "https://api.interfacedb.com/admin/storage/upload" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
...
```shell
curl -X DELETE "https://api.interfacedb.com/admin/storage/files?filename=snippets/demo.txt" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID"
```
...
```shell
curl -X POST "https://api.interfacedb.com/admin/storage/files/delete" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "App-Id: $APP_ID" \
