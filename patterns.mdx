...
attribute by adding this to your app's [permissions](https://interfacedb.com/dash?t=perms)
...
You can use the [`fields`](/instaql#select-fields) clause to restrict which attributes
are returned from the server, but this will not prevent a client from doing
another query to get the full entity.

You can set special permission rules for [`fields`](/instaql#select-fields) to make sure only certain attributes
...
First the [schema](/modeling-data):
...
Then the [permissions](/permissions):

```typescript
import type { InstantRules } from '@interface-db/react';
// instant.perms.ts
...
via a CDN through [unpkg](https://www.unpkg.com/@interface-db/core/).

```jsx
<!-- Load Instant via unpkg -->
<!-- Consider replacing @latest with a specific package -->
<script src="https://www.unpkg.com/@interface-db/core@latest/dist/standalone/index.umd.cjs"></script>
...
Once you have an ID, you can pass it around in your transactions and queries and use it in [ruleParams](/permissions#rule-params).

## Making admin queries work with Next.js caching

Next.js caches fetch requests and lets you revalidate them. [`adminDB.query`](/backend#query) uses fetch under the hood, so Next.js caching will work by default.
...
1. **Push schema/perms changes locally first**
   ...
   ```bash
   npx @interface-db/cli push --app your-local-app-id
   ```
...
3. **Push changes to production**
   ...
   ```bash
   npx @interface-db/cli push --app your-production-app-id
   ```
...
import { init, tx } from '@interface-db/admin';
...
```bash
npx tsx scripts/backfill-todos.ts && npx @interface-db/cli push
```
