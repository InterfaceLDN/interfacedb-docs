...
# Install instant
npm i @interface-db/react-native
...
```tsx
import { init, i, InstaQLEntity } from '@interface-db/react-native';
...
Huzzah 🎉 You've got your first React Native Instant app running! Check out the [Working with data](/init) section to learn more about how to use Instant!
...
```shell
npm i @interface-db/react-native-mmkv
...
Import `Store` from `@interface-db/react-native-mmkv` and pass it to `init`:

```tsx
import { init } from '@interface-db/react-native';
import Store from '@interface-db/react-native-mmkv';
...
```shell
npm i @interface-db/expo-sqlite
...
Import `Store` from `@interface-db/expo-sqlite` and pass it to `init`:

```tsx
import { init } from '@interface-db/react-native';
import Store from '@interface-db/expo-sqlite';
...
You can also implement your own local cache interface by extending `StoreInterface` from `@interface-db/react-native`. Here's an example in-memory store implementation:

```tsx
import {
  StoreInterface,
  StoreInterfaceStoreName,
} from '@interface-db/react-native';
...
[Building a Real-Time Chat App with Instant DB](https://www.youtube.com/watch?v=jyVR5NDjNJ4)
...
[Use React Native + InstantDB to Build a Self-Building App](https://www.youtube.com/watch?v=HRACNTmikZI)

If you prefer reading docs, you can also check out the [Working with data](/init) section to learn more about Instant concepts.
