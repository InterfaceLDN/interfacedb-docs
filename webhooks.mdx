...
You can also manage webhooks programmatically with `npx @interface-db/cli webhook` or through the Admin SDK:
...
import { init, Webhooks } from '@interface-db/admin';
import { sendNewPostEmail } from '@/lib/emails';
import schema from '@/instant.schema';
...
The `@interface-db/admin` SDK is the easiest way to receive webhooks, but the protocol is plain HTTP + Ed25519 — you can implement a receiver in any language. The steps below are what `validate` and `fetchPayloads` do under the hood.
...
- A JSON body containing a short-lived URL and JWT:
  ...
  ```json
  { "payloadUrl": "https://api.interfacedb.com/...", "token": "eyJ..." }
  ```
...
```text
https://api.interfacedb.com/.well-known/webhooks/jwks.json
```
...
JWKS_URL = "https://api.interfacedb.com/.well-known/webhooks/jwks.json"
...
const (
	jwksURL          = "https://api.interfacedb.com/.well-known/webhooks/jwks.json"
...
public class WebhookVerifier {
  static final String JWKS_URL = "https://api.interfacedb.com/.well-known/webhooks/jwks.json";
