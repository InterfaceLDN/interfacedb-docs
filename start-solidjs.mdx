...
The fastest way to get started with Instant and SolidJS is to use @interface-db/create-app to scaffold a new project with Instant already set up.

To get started, run:

```shell
npx @interface-db/create-app -b solidjs-vite
```
...
```ts
import { init } from '@interface-db/solidjs';
...
```ts
import { i } from '@interface-db/solidjs';
...
```typescript
import { id, type InstaQLEntity } from "@interface-db/solidjs";
...
Transactions in Solid work the same way they do in React via `db.transact`. To learn more, see our [writing data](/instaml) docs.
