...
Instant comes with support for auth. We currently offer [Magic Codes](/auth/magic-codes), [Guest Auth](/auth/guest-auth), [Google OAuth](/auth/google-oauth), [Sign In with Apple](/auth/apple), [GitHub OAuth](/auth/github-oauth), [LinkedIn OAuth](/auth/linkedin-oauth), and [Clerk](/auth/clerk). If you want to build your own flow, you can use the [Admin SDK](/backend#custom-auth).
...
- [Magic Codes](/auth/magic-codes): Send login codes to your users via email. Removes the need for passwords.
- [Guest Auth](/auth/guest-auth): Allow your users to try your app before they sign up.
- [Google OAuth](/auth/google-oauth): We provide flows for Web and React Native to enable Google OAuth for your app.
- [Sign In with Apple](/auth/apple): Sign In to native apps with Apple ID.
- [GitHub OAuth](/auth/github-oauth): Log in with GitHub on both Web and React Native.
- [LinkedIn OAuth](/auth/linkedin-oauth): Log in with LinkedIn on both Web and React Native.
- [Clerk](/auth/clerk): Integrate Clerk's auth flow with Instant.
- [Firebase Auth](/auth/firebase): Integrate Firebase's auth flow with Instant.
- [Custom Auth](/backend#custom-auth): Integrate your own auth flow with the Admin SDK.
...
If you want to set custom properties on `$users` at signup time, see [Setting properties at signup](/users#setting-properties-at-signup).
