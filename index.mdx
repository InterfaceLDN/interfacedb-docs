...
InterfaceDB is an independently operated fork of Instant. Use `@interface-db/*` packages and an approved InterfaceDB account. Start with the [InterfaceDB connection guide](/sdk) for endpoints and access requirements. The guides below retain upstream terminology; hosted-service plans and availability can differ.
...
The fastest way to get started with Instant is to use `npx @interface-db/create-app` to scaffold a new project with Instant already set up.

To get started with Next.js run:

```bash
npx @interface-db/create-app --next
```

To get started with Vite run:

```bash
npx @interface-db/create-app --vite-react
```
...
```ts
import { i } from '@interface-db/react';
...
import { id, i, init, InstaQLEntity } from "@interface-db/react";
...
Want to dive deeper on how this todo app works? Check out our step-by-step [Todo List Tutorial](https://interfacedb.com/examples/todos). In this tutorial we walk through how to build the above todo list app from scratch, and explain how the queries and transactions work in more detail.

You can also check out the [Working with data](/init) section to learn more about Instant concepts.

As you get more familiar with Instant, check out our [Recommended Workflow](/workflow) docs for using Instant in your projects.
