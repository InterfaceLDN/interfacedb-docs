...
As your app grows, you may want to start enforcing types. When you're ready, you can start using a [schema](/modeling-data). If your schema includes a `todos.dueDate` for example:
...
![Documentation screenshot](/img/docs/instaml-due-date.png)
...
```typescript
import { UpdateParams } from '@interface-db/react';
...
```typescript
import { LinkParams } from '@interface-db/react';
import { AppSchema } from '../instant.schema.ts';
