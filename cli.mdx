...
```shell
npx @interface-db/cli@latest login
```
...
```shell
npx @interface-db/cli@latest logout
```
...
```shell
npx @interface-db/cli@latest init
```
...
```shell
npx @interface-db/cli@latest init --temp
```
...
To learn how to change `instant.schema.ts`, check out our [Modeling Data](/modeling-data). For `instant.perms.ts`, check out the [permissions](/permissions) page.
...
```shell
npx @interface-db/cli@latest push schema
```
...
```shell
npx @interface-db/cli@latest push perms
```
...
```shell
npx @interface-db/cli@latest push schema --rename posts.author:posts.creator
```

The format is `oldName:newName`. You can rename multiple fields at once:

```shell
npx @interface-db/cli@latest push schema --rename posts.author:posts.creator stores.owner:stores.manager
```

This is especially useful when combined with `--yes` to skip confirmation prompts:

```shell
npx @interface-db/cli@latest push schema --yes --rename posts.author:posts.creator
```
...
```shell
npx @interface-db/cli@latest pull
```
...
```shell
npx @interface-db/cli@latest query '{ posts: { comments: {} } }'
```
...
```shell
npx @interface-db/cli@latest query --as-email alice@example.com '{ posts: {} }'
```
...
```shell
  npx @interface-db/cli@latest init --app $MY_APP_ID
```
...
```shell
npx @interface-db/cli@latest login -p
```
...
```shell
npx @interface-db/cli@latest init-without-files --title "Hello World"
```
...
```shell
npx @interface-db/cli@latest init-without-files --title "Hello World" --temp
```

You can also pipe the output of this command to `jq` to extract the app information for use in scripts:

```shell
output=$(npx @interface-db/cli@latest init-without-files --title "Hello World" --temp)
...
npx @interface-db/cli@latest auth client list

# Upgrade a Google dev client to your own credentials
npx @interface-db/cli@latest auth client update \
  --name google-web \
  --client-id <id> --client-secret <secret>

# Add a redirect origin
npx @interface-db/cli@latest auth origin add \
...
```shell
npx @interface-db/cli@latest auth email pull
```
...
Just like on the dashboard, you can use the [variables available for the subject line and body](/emails#variables)
